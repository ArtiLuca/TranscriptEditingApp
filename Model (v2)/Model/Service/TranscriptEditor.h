...
/**
 * @brief Service class providing editing operations on a single Transcript.
 *
 * Responsibilities:
 *  - Segment-level editing (insert, delete, move, merge, split, change text)
 *  - Speaker-level editing (change segment speaker, rename speaker globally)
 *  - Text operations (find/replace, normalize whitespace)
 *  - Basic undo/redo for editing actions (snapshot-based)
 *
 * This class operates on an existing Transcript instance and does not perform
 * any file I/O. Persistence is handled by TranscriptManager / TranscriptExporter.
 */
...
public:
    ...
    /**
     * @brief Splits a segment into two parts with possibly different speakers.
     *
     * The original segment at @p index is split at @p splitPosition.
     * The first part gets @p speakerFirst (or keeps the original speaker if empty),
     * the second part gets @p speakerSecond (or keeps the original speaker if empty).
     *
     * Returns the index of the new second segment on success, -1 on error.
     */
    int splitSegmentWithSpeakers(int index,
                                 int splitPosition,
...
#ifdef QT_DEBUG
    /** @brief Logs current undo/redo sizes (debug only). */
    void debugLogStacks(const char* context) const;
#endif
...
#ifdef QT_DEBUG
    void debugDumpSegment(int index, const char* context) const;
#endif
